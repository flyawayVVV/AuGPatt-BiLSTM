...
# 【修改】：类名改为标准的 BiLSTM，且内部去除了 Attention
class BiLSTM(nn.Module):
    def __init__(self, input_size, hidden_size, num_layers, output_size, dropout_rate=0.0): # 修改：增加 dropout_rate
        super(BiLSTM, self).__init__()
        self.hidden_size = hidden_size
        self.num_layers = num_layers

        # 标准 BiLSTM 层 - 修改：激活 dropout
...
def train_bilstm(model, train_data, train_labels, epochs, batch_size, learning_rate, weight_decay): # 修改：增加 weight_decay
    criterion = nn.MSELoss()
...
def fitness_function(hyperparameters):
    ...
    # 【修改】：实例化 BiLSTM 时传入 dropout_rate
    bilstm = BiLSTM(input_size, hidden_size, num_layers, output_size, dropout_rate)
    bilstm.to(device)

    # 【修改】：训练时传入 weight_decay
    ...
    with open('gsa_pso_results_bilstm.txt', 'a') as f:
...
if WeatherRunGSA_PSO == 1:
    print("Starting GSA-PSO optimization (Standard BiLSTM)...")
    ...
    best_hyperparameters = [5, 128, 0.0008954796058415571, 0.09384546784196496, 2.734484404526616e-07, 3]

print("Current hyperparameters:", best_hyperparameters)
...
# 【修改】：实例化 BiLSTM 时传入 best_dropout_rate
...
excel_path = os.path.join(save_path, 'observed_vs_predicted_AnimasRiver_AuGP_BiLSTM.xlsx')
observed_vs_predicted.to_excel(excel_path, index=False, engine='openpyxl')
print(f"Results saved to {excel_path}")

# 保存模型权重
save_file_ae = os.path.join(save_path, 'Autoencoder_AuGP_BiLSTM.pth')
with open(save_file_ae, 'wb') as f:
    torch.save(autoencoder.state_dict(), f)

save_file_lstm = os.path.join(save_path, 'AuGP_BiLSTM.pth')
